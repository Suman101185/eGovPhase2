...
<script runat="server">
    ...
    {
        sma = VB.space(79);
        rev_func_srv.PopulateBuffer(xmlReqDoc, "WAGE_INFO", ewage,ref gsDebugUp);
        rev_func_srv.PopulateBuffer(xmlReqDoc, "AMT_INFO", amt,ref gsDebugUp);
        bool bLoop = false;
        int iTry = 0;
        do
        {
            a = objWageReturn.RTTIE027(ref sma, ref ewage, ref amt);
            if (objWageReturn.ErrorNumber == 148)
            {
            }
            else if (objWageReturn.ErrorNumber != 0)
            {
                rev_func_srv.DoError("WageReturn", "5020003", "", "SAG: ", xmlOut, msApp);
                bLoop = true;
        ...
        if (iTry == 5)
        {
            rev_func_srv.DoError("WageReturn", "5010004", "",
                    "SAG: Mainframe is busy, Submit again...", xmlOut, msApp);
        }

        if (objWageReturn.ErrorNumber == 0)
        {


            rev_func_srv.PopulateXML("ERROR_INFO", sma, xmlOut, xmlOut);
            rev_func_srv.PopulateXML("WAGE_INFO", ewage, xmlOut, xmlOut);
            rev_func_srv.PopulateXML("AMT_INFO", amt, xmlOut, xmlOut);

        }

        xmlProc.ispXmlSetFieldVal(xmlOut, sma + ewage + amt, "DEBUG", "", 0);
...
<%
    ...
    rev_func_srv.CreateXmlBufferObj(xmlOut, util.GetXMLString(Server.MapPath("xml/WageReturn.xml")), false);
    rev_func_srv.CreateXmlBufferObj(xmlReqDoc, util.GetXMLString(Request), false);

    inAcctNo = xmlProc.ispXmlGetFieldVal(xmlReqDoc, "WAGE_INFO/ENTITY_ID", "", 0);
    lsFunction = xmlProc.ispXmlGetFieldVal(xmlReqDoc, "WAGE_INFO/RETURN_STATUS", "", 0);

    objWageReturn = (CopRevenueGov2.Service.EOL_WageForm)VB.CreateObject("EOL.WageForm");

    if (objWageReturn == null)
    {
        rev_func_srv.DoError("WageForm", "5010001", "",
                "Object not loaded: 'EOL.WageForm'", xmlOut, msApp);
    }

    objWageReturn.ServerAddress = CopMvcUtil.GetString(Session["SagAddress"]);

    CallRttie027();

    if (xmlProc.ispXmlGetFieldVal(xmlOut, "WAGE_INFO/ENTITY_ID", "", 0) != "")
    {

        xmlProc.ispXmlSetFieldVal(xmlOut, inAcctNo.ToString(), "WAGE_INFO/ENTITY_ID", "", 0);
    }

    xmlProc.ispXmlSetFieldVal(xmlOut, lsFunction, "WAGE_INFO/RETURN_STATUS", "", 0);
