...
<link rel="Stylesheet" href="../Content/Styles/Object.css" />
    ...
    <script language="JavaScript" type="text/javascript">
        ...
        function DisplayPrntCoupon() {
            ...
            var count = 0;
            var entid = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO ENTITY_ID', '', 0);
            var account;
            //  alert((new XMLSerializer()).serializeToString(parent.MenuWin.xmlPayCoupon));
            $(parent.MenuWin.xmlPayCoupon).find("TAX_ACCT ACCOUNT_ID").each(function () {
            ...
            //  if (ispXmlGetRecCount(parent.MenuWin.xmlPayCoupon, 'TAX_ACCT ACCOUNT_ID = ' + ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO ENTITY_ID', '', 0) + ', 'n') == 1)
            if (count == 1) {
                ...
                //alert('in');
                //alert(olXML);
                //            olXML.loadXML(ispXmlGetRecordXml(parent.MenuWin.xmlPayCoupon, 'TAX_ACCT[ACCOUNT_ID = ' + ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO ENTITY_ID', '', 0) + ']'))

                // var olXML = ispXmlGetRecordXml(parent.MenuWin.xmlPayCoupon, 'TAX_ACCT[ACCOUNT_ID = ' + ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO ENTITY_ID', '', 0) + ']')


                for (var i = 0; i < ddPCAcctType.options.length; i++) {

                    // if ($(ddPCAcctType.options[i]).attr("CODE") == ispXmlGetFieldVal(olXML, 'ACCOUNT', '', 0))
                ...
                for (var i = 0; i < ddPCAcctID.options.length; i++) {
                    if (ddPCAcctID.options[i].innerText == ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO ENTITY_ID', '', 0)) {
            ...
            if (ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'LOGIN_FROM NTL_ONLY', '', 0) == 'TRUE') {
        ...
        function FixLeadingZeroForSSN() {
            ...
            var icount = ispXmlGetRecCount(parent.MenuWin.xmlPayCoupon, "TAX_ACCT", "");

            var sAcctId = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ACCT_ID", "", 0);

            var sAccount_Id;

            for (i = 0; i < icount; i++) {
                sAccount_Id = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT_ID", "", i);

                if (sAccount_Id == sAcctId.substr(1)) {
                    ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, sAcctId, "TAX_ACCT ACCOUNT_ID", "", i);
        ...
        function LoadPCAccountType() {
            ...
            for (i = 0; i < icount; i++) {
                if (ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT", "", i) == '85') {
                    //Do not load UO - Tenant (85)
                } else if (ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT", "", i) == lastUniqueCode) {
                    lastUniqueCode = lastUniqueCode;
                } else {
                    lastUniqueCode = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT", "", i);
                    ...
                    if (lastUniqueCode > 0) {
                        ...
                        if (lastUnique == strUniqueText) {
                            ...
                            $(ddPCAcctType.options[iunique]).attr("CODE", ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT", "", i));
            ...
            //	if (ispXmlGetRecCount(parent.MenuWin.xmlPayCoupon, 'TAX_ACCT[ACCOUNT_ID = ' + ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO/ENTITY_ID', '', 0) + ']', 'n')) {
            //ddPCAcctType.selectedIndex
            //	}		//if
            //  alert(ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO/ENTITY_ID', '', 0))
        }   //loadPCAccountType
        ...
        function LoadPCAccountID() {
            ...
            for (i = 0; i < icount; i++) {
                strAccountType = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT", "", i);

                if (strAccountType == strSelectType) {
                    strUniqueText = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT_ID", "", i);
                    if ($(ddPCAcctID.options[iunique]).text() == strUniqueText) {
                        ...
                        $(ddPCAcctID.options[iunique]).attr("CODE", ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT ACCOUNT_ID", "", i));
                        if (strSelectType == '60') {
                            strBRTAcct = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT BRT_ACCOUNT", "", i);
                            $(ddPCAcctID.options[iunique]).text(strBRTAcct);
                            $(ddAccountAddr.options[iunique]).text(ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT BRT_ADDRESS1", "", i));
                        } // if
            ...
            //	ValidatePrntCoupon()
            //	ddPCAcctID.options.innerText = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, 'ENTITY_INFO/ENTITY_ID', '', 0)
        ...
        function LoadPCTaxYear() {
            ...
            HoldDate = new Date(ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT START_DATE", "", iSelectIdx));

            iStartYear = HoldDate.getFullYear();

            HoldDate = new Date(ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT END_DATE", "", iSelectIdx));
        ...
        function LoadTaxPeriods() {
            ifreq = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "TAX_ACCT FREQUENCY_CODE", "", iSelectIdx);
        ...
        function SetupPassFields() {
            ...
            strAccountName = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO NAME", "");
            strAccountAddr = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ADDRESS1", "");
            var strAccountAddr2 = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ADDRESS2", "");
            var strAccountAddr3 = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ADDRESS3", "");
            var strAccountCity = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO CITY", "");
            var strAccountState = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO STATE", "");
            var strAccountZip = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ZIP_CODE", "");
            var strEntityID = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ENTITY_ID", "");
            ...
            if (strSelectType == '60') {
                ...
                strCouponAddr = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "ENTITY_INFO ADDRESS1", "");
            }  //if
            ...
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strSelectType, "COUPON_FORM CPN_ACCOUNT_TYPE", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAcctID, "COUPON_FORM CPN_ACCOUNT", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountName, "COUPON_FORM CPN_NAME", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountAddr, "COUPON_FORM CPN_ADDR", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strScanLine, "COUPON_FORM CPN_SCAN", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountAddr2, "COUPON_FORM CPN_ADDR2", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountAddr3, "COUPON_FORM CPN_ADDR3", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountCity, "COUPON_FORM CPN_CITY", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountState, "COUPON_FORM CPN_STATE", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAccountZip, "COUPON_FORM CPN_ZIP_CODE", "");
        ...
        function NTLPrntCoupon() {
            ...
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, $(ddPCAcctID.options[ddPCAcctID.selectedIndex]).text(), "WAGE_NTL ACCOUNT_ID", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, lsPeriod, "WAGE_NTL PERIOD", "");
            ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, ifreq, "WAGE_NTL FREQUENCY_CODE", "");
        ...
        function SubmitPrntCoupon() {
            ...
            var strAcctType = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_ACCOUNT_TYPE", "");
            var strAccountAddr = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_ADDR", "");
            var strAccountAddr2 = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_ADDR2", "");
            var strAccountAddr3 = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_ADDR3", "");
            var strAccountCity = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_CITY", "");
            var strAccountState = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_STATE", "");
            var strAccountZip = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon, "COUPON_FORM CPN_ZIP_CODE", "");
            ...
            parent.DocWin.location.href = 'PDFCouponCreate?' + tempParameters;
            //	ispCallXMLForm(parent.MenuWin.xmlPayCoupon, xmlTemp, 'PDFCouponCreate.asp?' + tempParameters, '')
            //	alert(xmlTemp)
