...
//refresh bptez form
    ...
    if ($(parent.MenuWin.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
		...
		ispXmlSetFieldVal(parent.MenuWin.xmlBPTez, '12/31/2010', 'BPT_INFO PERIOD', '', 0);
...
//suman set last parameter true will cause xml copy to browser clipboard
	...
	if (parent.MenuWin.GetInnerNodeText(parent.MenuWin.xmlAccount, 'TAX_ACCT ACCOUNT', 'START_DATE', '24').slice(-4) == '2011') {
	...
	if (txtBPTez2_3.value != ispFormatMoney(0,0).toString())	{ 
		for (i = 0; i < iCount; i++) {
			if	(ispXmlGetFieldVal(parent.MenuWin.xmlAccount, 'TAX_ACCT ACCOUNT', '', i) == '24') {
				...
				if	(new Date(ispXmlGetFieldVal(parent.MenuWin.xmlAccount, 'TAX_ACCT START_DATE', '', i)) >= new Date("01/01/2010"))	{
