...
<script id="clientEventHandlersJS" language="javascript" type="text/javascript">
    ...
    function chkTaxpayer_onclick() {
        ...
        var  id=mnuElement.id;
            ...
            if (id == 'chkTaxpayer' && $('#chkTaxpayer').is(":checked")==true ) {
                //alert('chkTaxpayer');
                $('#chkPreparer').removeAttr("checked");
                //$('#chkTaxpayer').attr("checked","checked");
               chkPreparer.checked = false;
            }
            else if (id == 'chkPreparer' && $('#chkPreparer').is(":checked")==true)
            
            {
                //alert('#chkPreparer');
        
            //	$('#chkTaxpayer').removeAttr("checked");
                //$('#chkPreparer').attr("checked","checked");
            //chkTaxpayer.checked = false;
    ...
    function ValidatePreparer() {
        ...
        if ($('#chkTaxpayer').attr("checked") == undefined && $('#chkPreparer').attr("checked") == undefined) {
        ...
        if ($('#chkTaxpayer').attr("checked") != "checked" && $('#chkPreparer').attr("checked") != "checked") {
        ...
        if ($('#chkAgreement').attr("checked") == undefined || $('#chkAgreement').attr("checked") != "checked") {
    ...
    ispXmlSetFieldVal(xmlStruct, '<% Response.Write(CopMvcUtil.GetString(Request.ServerVariables["REMOTE_ADDR"])); %>', xmlRegion + " PREPARER_IP_ADDRESS", '', 0);
    ispXmlSetFieldVal(xmlStruct, txtPreparerName.value, xmlRegion + " PREPARER_NAME", '', 0);
    ispXmlSetFieldVal(xmlStruct, txtPrepAreaCode.value + txtPrepPhone1.value + txtPrepPhone2.value, xmlRegion + " PREPARER_PHONE", '', 0);
    ispXmlSetFieldVal(xmlStruct, txtPrepPhoneExt.value, xmlRegion + " PREPARER_PHONE_EXT", '', 0);
    ispXmlSetFieldVal(xmlStruct, txtPreparerEmail.value, xmlRegion + " PREPARER_EMAIL_ADDRESS", '', 0);
    if ($('#chkTaxpayer').attr("checked") == "checked") {
        ispXmlSetFieldVal(xmlStruct, "T", xmlRegion + " PREPARER_WHO", '', 0);
    }
    else if ($('#chkPreparer').attr("checked") == "checked")
    {
        ispXmlSetFieldVal(xmlStruct, "P", xmlRegion + " PREPARER_WHO", '', 0);
    }		//if
...
function PopulatePreparer(xmlStruct, xmlRegion) {
    ...
    txtPreparerName.value = ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_NAME", '', 0);
    txtPrepAreaCode.value = splitField(ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_PHONE", '', 0), 0, 3);
    txtPrepPhone1.value = splitField(ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_PHONE", '', 0), 3, 3);
    txtPrepPhone2.value = splitField(ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_PHONE", '', 0), 6, 4);
    txtPrepPhoneExt.value = ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_PHONE_EXT", '', 0);
    txtPreparerEmail.value = ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_EMAIL_ADDRESS", '', 0);
    if (txtPreparerName.value != '') {
        if (ispXmlGetFieldVal(xmlStruct, xmlRegion + " PREPARER_WHO", '', 0) == 'T') {
            $('#chkTaxpayer').attr("checked", "checked");
...
function ClearPreparerXML(xmlStruct, xmlRegion) {
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_IP_ADDRESS", '', 0);
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_NAME", '', 0);
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_PHONE", '', 0);
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_PHONE_EXT", '', 0);
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_EMAIL_ADDRESS", '', 0);
    ispXmlSetFieldVal(xmlStruct, "", xmlRegion + " PREPARER_WHO", '', 0);
}		//ClearPreparerXML
...
<table id=TABLE1 style="MARGIN-LEFT: 0%" cellSpacing=0 cellPadding=0 width="80%" border=0>
  <TR>
    ...
    <INPUT id=chkTaxpayer type=checkbox style="WIDTH: 13px; HEIGHT: 20px; 
				MARGIN-LEFT: 10px; MARGIN-RIGHT: 10px" size=13 onclick="chkTaxpayer_onclick()"><label class="lblTextBlackSmall">&nbsp;Taxpayer </label>
		<INPUT id=chkPreparer type=checkbox style="WIDTH: 13px; HEIGHT: 20px;
				MARGIN-LEFT: 10px; MARGIN-RIGHT: 10px" size=13 onclick="chkTaxpayer_onclick()"><label class="lblTextBlackSmall">&nbsp;Preparer</label></td>
	</TR>
  <TR>
 ...
 <TR>
  ...
  <TR>
    ...
    <INPUT id="chkAgreement" type="checkbox" style="WIDTH: 13px; HEIGHT: 20px" size="13">&nbsp;Signature box: 
