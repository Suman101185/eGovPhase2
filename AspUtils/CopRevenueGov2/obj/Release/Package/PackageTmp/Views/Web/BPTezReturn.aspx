...
<script runat="server">
    ...
    {
        sma = VB.space(79);
        rev_func_srv.PopulateBuffer(xmlReqDoc, "BPT_INFO", ref ebpt);
        rev_func_srv.PopulateBuffer(xmlReqDoc, "BPT_PG1", ref page1);
        rev_func_srv.PopulateBuffer(xmlReqDoc, "BPT_PG2", ref page2);
        bool bLoop = false;
        int iTry = 0;
        do
        {
            a = objBPTezReturn.RTTIE037(ref sma, ref ebpt, ref page1,ref page2);
            
            if (objBPTezReturn.ErrorNumber == 148)
            {
            }
            else if (objBPTezReturn.ErrorNumber != 0)
            {
                rev_func_srv.DoError("BPTezForm", "5030003", "", "SAG: ", xmlOut, msApp);
                bLoop = true;
        ...
        if (iTry == 5)
        {
            rev_func_srv.DoError("BPTezForm", "5030004", "",
                    "SAG: Mainframe is busy, Submit again...", xmlOut, msApp);
        }

        if (objBPTezReturn.ErrorNumber == 0)
        {
            rev_func_srv.PopulateXML("ERROR_INFO", sma, xmlOut, ref xmlOut);
            rev_func_srv.PopulateXML("BPT_INFO", ebpt, xmlOut, ref xmlOut);
            rev_func_srv.PopulateXML("BPT_PG1", page1, xmlOut, ref xmlOut);
            rev_func_srv.PopulateXML("BPT_PG2", page2, xmlOut, ref xmlOut);
        }

        xmlProc.ispXmlSetFieldVal(xmlOut, sma + ebpt + page1 + page2, "DEBUG", "", 0);
