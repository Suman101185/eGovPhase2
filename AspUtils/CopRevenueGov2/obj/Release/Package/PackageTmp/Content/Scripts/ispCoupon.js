...
function SetupPassFields() {
	var strPayPeriod = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM CPN_PERIOD', '');
  var strAcctType = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM CPN_ACCOUNT_TYPE', '');
	var strAcctID = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM CPN_ACCOUNT', '');
	var strAccountName = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM CPN_NAME', '');
...
//  var strEntityID = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM/CPN_ENTITY_ID', '')
	var i = 0;
	var iTotal = 0;
	var iDigit = 0;
	var strScanID;
...
//  Format Period year
	twoyear = ispXmlGetFieldVal(parent.MenuWin.xmlPayCoupon,'COUPON_FORM CPN_YEAR', '')	;
//		  find 2 digit year
	if (twoyear > 2000) {
		while (twoyear > 999) {
			twoyear = twoyear - 1000;
...
//		create century code for SIT accounts (in CCYY year, century code is 2nd C)\
	...
	if (strAcctType == '11') {
		...
		strScanLine = '333' + strAcctType + HoldDate + '00' + '00000' + strScanID + '0000000000000000000' + centurySIT + twoyear + '000000000000000';
...
//	if (strAcctType == '60') {  // use entity for account in grid if Real Estate
//	strAcctID = strBRTAcct     
//	}  //if

/*	ispXmlSetFieldVal(parent.MenuWin.xmlPayCoupon, strAcctType, "COUPON_FORM/CPN_ACCOUNT_TYPE", "")
...
//	tempParameters = 'a=' + strAcctID + '&b=' + strAccountName + '&c=' + strAccountAddr + '&d=' + strScanLine + '&e=' + strPeriod + '&f' + strAcctType 
//	tempParameters = tempParameters.replace(/#/g, '~LBSIGN~')
