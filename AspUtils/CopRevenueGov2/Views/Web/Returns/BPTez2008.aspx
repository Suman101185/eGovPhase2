...
//refresh bptez form
	parent.$g.loadXmlSync('../XML/BPTezReturn',parent.MenuWin.parseXml_xmlBPTez);
	parent.$g.loadXmlSync('../XML/BPTezReturn',parent.MenuWin.parseXml_xmlBPTezTemplate);
	
    if ($(parent.$g.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
        parent.MenuWin.ispHighlightMenu(parent.MenuWin.divBPTezPage08); 	//.mnuBPTezYear10Page2
		...
		$('#AppHeader').html(txtHeader + '<Font class=hdrMedium>|Business Income & Receipts Tax-EZ for 2008</Font>');
		parent.MenuWin.gsInstructionItem = '#BptEz';
		$('#AppMessage').html(parent.MenuWin.gsInstructionText 
                              + '<BR>The Business Income & Receipts Tax for 2008 is due April 15, 2009.');
...
//suman set last parameter true will cause xml copy to browser clipboard
	...
	if (parent.MenuWin.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'START_DATE', '24').slice(-4) == '2009') {
...
function CheckBPTezXML(){
	...
	if (bPeriodExist == false) {
	    ...
	    parent.$x.ispXmlSetFieldVal(parent.$g.xmlBPTez, '12/31/2007', 'BPT_INFO PERIOD', '', 0);
...
function btnBPTezSubmit_onclick() {
	...
	if ($('#AppError').text() == "") {
		...
		parent.DocWin.ispCallXMLForm(parent.$g.xmlBPTez, parent.$g.xmlBPTez, "../BPTezReturn", "");
	...
	if (txtBPTez2_3.value != ispFormatMoney(0,0).toString())	{ 
		for (i = 0; i < iCount; i++) {
			if	(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '', i) == '24') {
				...
				if	(new Date(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT START_DATE', '', i)) >= new Date("01/01/2008"))	{
				    ...
				    alert('This return is not eligible for Loss Carry Forward, because your BPT Start Date '
                         + BPTStartDate 
                         + ' is on or after 01/01/2008. Loss Carry Forward (Page 2, Question #3) has been set to zero.');
				}
			}
		}
	}
...
function CalcPage1Line9_10() {
    ...
    if(cCur($('#lblBPTez1_9').text()) < 0){
        ...
        if( totalBPTez1_10Amount !=  Math.abs( cCur($('#lblBPTez1_7').text()))){
            $('#AppError').text( "Lines 10a, 10b && 10c must equal Line 7.");
        }
...
<body onload="ispDisplayProgress('DisplayBPTezReturn()', 'Retrieving 2008 business privilege tax information')">
...
<!--	<td colspan=6>
		<label class="lblTextBlacksmall">If your business terminated in 2006,<br>
				enter the termination date here:</label></td>-->
<!--		<label class="lblInstrNormal">From</label>
		<input class=inpNormal id=txtFromDate onchange="ValidateBPTezReturn()" maxlength=10 
			onfocus="SetFocus()" onblur="return txtFromDate_onblur()" 
      style="WIDTH: 68px; HEIGHT: 22px" size=9>
		<label class="lblInstrNormal">To </label>-->
<!--	<td colspan=2 style="VERTICAL-ALIGN: bottom">
		<input class=inpNormal    
			 id=txtTermDate onchange="ValidateBPTezReturn()" onfocus="SetFocus()" maxlength=10 onblur="return txtTermDate_onblur()" 
     ></td>-->
	</tr>
</table>
...
<table class=tblShowBorder cellSpacing=0 cellPadding=0 border=0 style="MARGIN-LEFT: 4%" id=TABLE1  width="90%">
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Tax Due for the 2008 Business Privilege Tax</label></td>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			<label class="lblInstrNormal"><B><U>MANDATORY</U> 2009 BPT Estimated Payment <u>(repeat line 3)</u></label></B></td>
		<td colspan=3>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Total Due by <B>4/15/2009</B></label></td>	
		<td colspan=8 align=right>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Credit from overpayment of 2007 or 2008 Net Profit Tax</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
			<label class="lblInstrNormal">Applied, up to the tax due,	to the 2008 Net Profits Tax Return</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			<label class="lblInstrNormal">Applied to the 2009	Business Privilege Tax</label></td>
		<td colspan=3>
...
<!--<table style="MARGIN-LEFT: 4%" border=0  width="90%">
  <TR>
    ...
    <INPUT id=chkTaxpayer type=checkbox style="WIDTH: 13px; HEIGHT: 20px" size=13
			 onclick="chkTaxpayer_onclick()">&nbsp;Prepared by Taxpayer</label></td>
	</TR>
</table>
<table style="MARGIN-LEFT: 4%" border=0  width="95%">
  <TR>
    <TD  width ="5%" nowrap><label class="lblTextBlackSmall">&nbsp;Preparer Name:&nbsp;
		<INPUT class=inpNormal id=txtPreparerName   
			 maxLength=40 size=48 name=txtPreparerName></TD> 
    <td width ="5%" nowrap><label class="lblTextBlackSmall">
			 &nbsp;Preparer Phone:&nbsp;
		<INPUT class=inpNormal id=txtPrepAreaCode name=txtPrepAreaCode maxlength=3 size=4>
		<INPUT class=inpNormal id=txtPrepPhone1 name=txtPrepPhone1 maxlength=3 size=4>
		<INPUT class=inpNormal id=txtPrepPhone2 name=txtPrepPhone2 maxlength=4 size=5>&nbsp;Ext:&nbsp;</FONT>
		<INPUT class=inpNormal id=txtPrepPhoneExt name=txtPrepPhoneExt maxlength=5 size=6></TD>
	</TR>
	</table>
 <table style="MARGIN-LEFT: 4%" border=0  width="95%">
 <TR>
    <TD  width ="5%" nowrap><label class="lblTextBlackSmall">&nbsp;Preparer E-Mail Address:&nbsp;
		<INPUT class=inpNormal id=txtPreparerEmail   
			 maxLength=60 size=68 name=txtPreparerEmail></TD> 
	</TR>
</table>	-->
<table style="MARGIN-LEFT: 4%" border=0  width="90%">
	<tr>
...
<!--  <TR>
    <td width ="80%"><label class="lblTextBlackSmall">
    <INPUT id=chkAgreement type=checkbox style="WIDTH: 13px; HEIGHT: 20px" size=13
			 onclick="chkAgreement_onclick()">&nbsp;Signature box: 
		...
		<input id="btnBPTezPrint" type="button" value="Print" LANGUAGE=javascript onclick="return btnBPTezPrint_onclick()">
		<input id="btnBPTezSubmit" type="button" value="Submit" LANGUAGE=javascript onclick="return btnBPTezSubmit_onclick()"></p>
		</td>
	</TR>	-->
</table>
</div>
