...
<script runat="server">
    ...
    CopRevenueGov2.Service.EOL_LossCarryForward objLcfReturn = new CopRevenueGov2.Service.EOL_LossCarryForward();
    ...
    void CallRttie031_New()
    {
        xmlOut = RTTIE031.CallService(xmlReqDoc);
    ...
    //    do
    //    {
    //        a = objLcfReturn.RTTIE031(ref sma, ref eLcf);
    //        if (objLcfReturn.ErrorNumber == 148)
    //        {
    //        }
    //        else if (objLcfReturn.ErrorNumber != 0)
    //        {
    //            rev_func_srv.DoError("LosCarryForwardReturn", "5030003", "", "SAG: ", xmlOut, msApp);
    //            bLoop = true;
    ...
    //    if (iTry == 5)
    //    {
    //        rev_func_srv.DoError("objLcfReturn", "5030004", "",
    //                "SAG: Mainframe is busy, Submit again...", xmlOut, msApp);
    //    }

    //    if (objLcfReturn.ErrorNumber == 0)
    //    {


    //        rev_func_srv.PopulateXML("ERROR_INFO", sma, xmlOut, xmlOut);
    //        rev_func_srv.PopulateXML("LCF_INFO", eLcf, xmlOut, xmlOut);
           
    //    }

    //    xmlProc.ispXmlSetFieldVal(xmlOut, sma + eLcf , "DEBUG", "", 0);
    //    Response.Write(xmlProc.XMLToString(xmlOut));
    //}
...
<%
    ...
    rev_func_srv.CreateXmlBufferObj(xmlOut, util.GetXMLString(Server.MapPath("~/views/web/xml/LossCarryForward.xml")), false);
    rev_func_srv.CreateXmlBufferObj(xmlReqDoc, util.GetXMLString(Request), false);

    inAcctNo = xmlProc.ispXmlGetFieldVal(xmlReqDoc, "LCF_INFO/ACCOUNT_ID", "", 0);
    lsFunction = xmlProc.ispXmlGetFieldVal(xmlReqDoc, "LCF_INFO/YEAR_X", "", 0);

    objLcfReturn = (CopRevenueGov2.Service.EOL_LossCarryForward)VB.CreateObject("EOL.LossCarryForward");

    if (objLcfReturn == null)
    {
        rev_func_srv.DoError("LossCarryForwardReturn", "5030001", "",
                "Object not loaded: 'EOL.BPTezForm'", xmlOut, msApp);
    }

    objLcfReturn.ServerAddress = CopMvcUtil.GetString(Session["SagAddress"]);

    CallRttie031_New();

    if (xmlProc.ispXmlGetFieldVal(xmlOut, "LCF_INFO/ACCOUNT_ID", "", 0) != "")
    {

        xmlProc.ispXmlSetFieldVal(xmlOut, inAcctNo.ToString(), "LCF_INFO/ACCOUNT_ID", "", 0);
    }
