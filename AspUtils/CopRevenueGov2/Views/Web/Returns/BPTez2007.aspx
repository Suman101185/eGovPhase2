...
//refresh bptez form
    ...
    if ($(parent.$g.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
        parent.MenuWin.ispHighlightMenu(parent.MenuWin.mnuBPTezYear07); 	//.mnuBPTezYear10Page2
		...
		$('#AppHeader').html(txtHeader + '<Font class=hdrMedium>|Business Income & Receipts Tax-EZ for 2007</Font>');
		parent.MenuWin.gsInstructionItem = '#BptEz';
		$('#AppMessage').html(parent.MenuWin.gsInstructionText 
                              + '<BR>The Business Income & Receipts Tax for 2007 is due April 15, 2008.');
...
//suman set last parameter true will cause xml copy to browser clipboard
	...
	if (parent.MenuWin.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'START_DATE', '24').slice(-4) == '2008') {
...
//		parent.MenuWin.ispHighlightMenu(parent.MenuWin.mnuBPTezYear07Page1)
	    $('#BPTez_pg1').css("display", "block");
	    $('#BPTez_pg2').css("display", "none");
	}
}//ShowPage
	...
	if (txtBPTez2_3.value != ispFormatMoney(0,0).toString())	{ 
		for (i = 0; i < iCount; i++) {
			if	(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '', i) == '24') {
				...
				if	(new Date(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT START_DATE', '', i)) >= new Date("01/01/2007"))	{
				    ...
				    alert('This return is not eligible for Loss Carry Forward, because your BPT Start Date '
                         + BPTStartDate 
                         + ' is on or after 01/01/2007. Loss Carry Forward (Page 2, Question #3) has been set to zero.');
				}
			}
		}
	}
...
<body onload="ispDisplayProgress('DisplayBPTezReturn()', 'Retrieving 2007 business privilege tax information')">
...
<table class=tblShowBorder cellSpacing=0 cellPadding=0 border=0 style="MARGIN-LEFT: 4%" id=TABLE1  width="90%">
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Tax Due for the 2007 Business Privilege Tax</label></td>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			<label class="lblInstrNormal"><B><U>MANDATORY</U> 2008 BPT Estimated Payment <u>(repeat line 3)</u></label></B></td>
		<td colspan=3>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Total Due by <B>4/16/2008</B></label></td>	
		<td colspan=8 align=right>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Credit from overpayment of 2006 or 2007 Net Profit Tax</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Include any estimated and/or extension payments of 2007 BPT previously
			  made,<br>and any credit from overpayment of the 2006 BPT return</label>
		</td>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			...
			<label class="lblInstrNormal">Applied to the 2008	Business Privilege Tax</label></td>
		<td colspan=3>
