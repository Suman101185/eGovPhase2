...
<script id="clientEventHandlersJS" language="javascript" type="text/javascript">
    ...
    function DisplayBPTezReturn() {
        ...
        //refresh bptez form
        parent.$g.loadXmlSync('XML/BPTezReturn', parent.$g.parseXml_xmlBPTez);
        parent.$g.loadXmlSync('XML/BPTezReturn', parent.$g.parseXml_xmlBPTezTemplate);

        if ($(parent.$g.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
           // parent.MenuWin.ispHighlightMenu(parent.MenuWin.mnuBPTezYear13); 	//.mnuBPTezYear10Page2
            ...
            $('#AppHeader').html(txtHeader + '<Font class=hdrMedium>|Business Income & Receipts Tax-EZ for 2013</Font>');
            //parent.MenuWin.gsInstructionItem = '#BptEz';// ? SUMAN Questionable mod for L & C
            parent.$g.gsInstructionItem = '#BIR';
            $('#AppMessage').html(parent.$g.gsInstructionText
                                  + '<BR>The Business Income & Receipts Tax for 2013 is due April 15, 2014.');
            ...
            if ($(parent.$g.divAcctInfo).css("display") == 'none') {
                $(parent.$g.divAcctInfo).css("display", "block");
                $(parent.$g.divAccount).css("display", "none");
            }

        }
        else {
            parent.$g.xmlBPTez = parent.$g.xmlBPTezTemplate;
            parent.$g.ispMenu_onclick(parent.$g.mnuAcctInfo);
        }
    ...
    */
    ...
    function getActiveElement() {
        ...
        var source = parent.$g.getEventSource(evt);
        //alert('event');
        return source;
    }
    ...
    function GoToPage1Button() {
        ...
        if (parent.$x.ispXmlGetFieldVal(parent.$g.xmlBPTez, 'BPT_INFO PREPARER_NAME ', "", 0) == 0 && iReceiptTaxDue == 0)//Sanghamitra:added this if block to disable the "go to page1" button in case of fresh form and it wont get activated untill there is value in Page1, line 7 
        {
            ...
            if (iIncomeTaxDue > 0 && iReceiptTaxDue == 0) {
                $('#mnuBPTezYear06Page1', parent.$g).attr("disabled", "disable");
                ...
                $('#mnuBPTezYear06Page1', parent.$g).removeAttr("disabled");
                $('#btnGoToPage1').removeAttr("disabled");
                // $('#AppError').text("");
...
<table class=tblShowBorder cellSpacing=0 cellPadding=0 border=0 style="MARGIN-LEFT: 4%" id=TABLE2  width="90%">
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
			...
			<label id="lbl1forpage2" class="lblLink" style="FONT-SIZE: 7pt" onclick=ShowPage(2)>Click here for Page 2</label>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			...
			<label id="lbl2forpage2" class="lblLink" style="FONT-SIZE: 7pt" onclick=ShowPage(2)>Click here for Page 2</label>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Tax Due for the 2013 Business Income & Receipts Tax</label></td>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Total Due by <B>4/15/2014</B></label></td>	
		<td colspan=8 align=right>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Credit from overpayment of 2012 or 2013 Net Profit Tax</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Include any estimated and/or extension payments of 2013 BIRT previously
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
			<label class="lblInstrNormal">Applied, up to the tax due,	to the 2013 Net Profits Tax Return</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			<label class="lblInstrNormal">Applied to the 2014	Business Income & Receipts Tax</label></td>
		<td colspan=3>
...
<table class=tblShowBorder cellSpacing=0 cellPadding=0 border=0 style="MARGIN-LEFT: 4%" id=TABLE3  width="90%">
  <tr height=0>
    <td WIDTH="5%"></td>
    <td WIDTH="5%"></td>
    <td WIDTH="5%"></td>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
			<label class="lblInstrNormal">Loss Carry Foward, if any. (<u><font style="cursor:pointer;" onclick="parent.MenuWin.InstructionLinks()">See Instruction Sheet IV</font></u>)</label>
