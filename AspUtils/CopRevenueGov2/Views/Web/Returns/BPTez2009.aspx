...
//refresh bptez form
    ...
    if ($(parent.$g.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
        parent.MenuWin.ispHighlightMenu(parent.MenuWin.mnuBPTezYear09); 	//.mnuBPTezYear10Page2
		...
		parent.$x.ispXmlSetFieldVal(parent.$g.xmlBPTez, '12/31/2009', 'BPT_INFO PERIOD', '', 0);

		$('#AppHeader').html(txtHeader + '<Font class=hdrMedium>|Business Income & Receipts Tax-EZ for 2009</Font>');
		parent.MenuWin.gsInstructionItem = '#BptEz';
		$('#AppMessage').html(parent.MenuWin.gsInstructionText 
                              + '<BR>The Business Income & Receipts Tax for 2009 is due April 15, 2010.');
...
//suman set last parameter true will cause xml copy to browser clipboard
	...
	if (parent.MenuWin.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'START_DATE', '24').slice(-4) == '2010') {
	...
	if (txtBPTez2_3.value != ispFormatMoney(0,0).toString())	{ 
		for (i = 0; i < iCount; i++) {
			if	(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '', i) == '24') {
				...
				if	(new Date(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT START_DATE', '', i)) >= new Date("01/01/2009"))	{
				    ...
				    alert('This return is not eligible for Loss Carry Forward, because your BPT Start Date '
                         + BPTStartDate 
                         + ' is on or after 01/01/2009. Loss Carry Forward (Page 2, Question #3) has been set to zero.');
				}
			}
		}
	}
...
<body onload="ispDisplayProgress('DisplayBPTezReturn()', 'Retrieving 2009 business privilege tax information')">
