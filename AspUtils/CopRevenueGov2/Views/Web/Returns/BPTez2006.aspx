...
//refresh bptez form
    ...
    if ($(parent.$g.xmlAccount).find("TAX_ACCT").find("ACCOUNT:contains('24')")) {
        parent.MenuWin.ispHighlightMenu(parent.MenuWin.mnuBPTezYear06); 	//.mnuBPTezYear10Page2
		...
		parent.$x.ispXmlSetFieldVal(parent.$g.xmlBPTez, '12/31/2006', 'BPT_INFO PERIOD', '', 0);

		$('#AppHeader').html(txtHeader + '<Font class=hdrMedium>|Business Income & Receipts Tax-EZ for 2006</Font>');
		parent.MenuWin.gsInstructionItem = '#BptEz';
		$('#AppMessage').html(parent.MenuWin.gsInstructionText 
                              + '<BR>The Business Income & Receipts Tax for 2006 is due April 15, 2007.');
...
//suman set last parameter true will cause xml copy to browser clipboard
	...
	if (parent.MenuWin.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'START_DATE', '24').slice(-4) == '2007') {
...
function CheckBPTezXML(){
	...
	if (bPeriodExist == false) {
	    ...
	    parent.$x.ispXmlSetFieldVal(parent.$g.xmlBPTez, '12/31/2005', 'BPT_INFO PERIOD', '', 0);
	...
	if (txtBPTez2_3.value != ispFormatMoney(0,0).toString())	{ 
		for (i = 0; i < iCount; i++) {
			if	(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '', i) == '24') {
				...
				if	(new Date(parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'TAX_ACCT START_DATE', '', i)) >= new Date("01/01/2006"))	{
				    ...
				    alert('This return is not eligible for Loss Carry Forward, because your BPT Start Date '
                         + BPTStartDate 
                         + ' is on or after 01/01/2006. Loss Carry Forward (Page 2, Question #3) has been set to zero.');
				}
			}
		}
	}
...
<body onload="ispDisplayProgress('DisplayBPTezReturn()', 'Retrieving 2006 business privilege tax information')">
...
<table class=tblShowBorder cellSpacing=0 cellPadding=0 border=0 style="MARGIN-LEFT: 4%" id=TABLE1  width="90%">
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Tax Due for the 2006 Business Privilege Tax</label></td>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
			<label class="lblInstrNormal"><B><U>MANDATORY</U> 2007 BPT Estimated Payment <u>(repeat line 3)</u></label></B></td>
		<td colspan=3>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=8>
			<label class="lblInstrNormal">Total Due by <B>4/16/2007</B></label></td>	
		<td colspan=8 align=right>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Credit from overpayment of 2005 or 2006 Net Profit Tax</label></td>
		<td colspan=3>
  ...
  <tr class=rowDataOdd>
		...
		<td colspan=16>
				<label class="lblInstrNormal">Include any estimated and/or extension payments of 2006 BPT previously
			  made,<br>and any credit from overpayment of the 2005 BPT return</label>
		</td>
  ...
  <tr class=rowDataEven>
		...
		<td colspan=16>
			<!--<INPUT type="radio" id=radBPTez1_10 name=radBPTez1_10 onclick="ValidateBPTezReturn()">-->
			<label class="lblInstrNormal">Applied, up to the tax due,	to the 2006 Net Profits Tax Return</label></td>
		<td colspan=3>
			<input class=inpNormal id=txtBPTez1_10b onchange="ValidateBPTezReturn()"
					onfocus="SetFocus()" maxlength=10 style="WIDTH: 100%"></td>
<!--			<label class=inpReadOnlyRt id=lblBPTez1_10b style="WIDTH: 100%"></label></td>-->
	</tr>
  <tr class=rowDataOdd>
		<td >
		...
		<td colspan=16>
			<!--<INPUT type="radio" id=radBPTez1_10 name=radBPTez1_10 onclick="ValidateBPTezReturn()">-->
			<label class="lblInstrNormal">Applied to the 2007	Business Privilege Tax</label></td>
		<td colspan=3>
