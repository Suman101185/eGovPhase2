...
//$(parent.NavWin.document).find('#tblNavMenu').first().attr("disabled")
	if ($(parent.document).find('#tblNavMenu').first().is(":disabled") == true) {
		return;
	}
	//if
	//alert(window.event);
	...
	if (event != null) {
		var eElement =  parent.getEventSource(event);
		if (eElement != null) {
		...
		$(parent.document).find('#btn7').first().css("visibility", "hidden");
	}		
...
function ispHideProgress(lLayer) {
	if (gProcessSubmitElement != null) {
		if (gProcessSubmitElement != '') {
			...
			$(parent.document).find('#btn7').first().css("visibility", "visible");
		}		
	}		
