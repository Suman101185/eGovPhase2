...
<script language="javascript" type="text/javascript">
    ...
    function DisplayNPTReturn() {
        ...
        if (parent.GetNodeCount(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '3') == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2015","ServiceName" : "RTTIE039","RequestXML" : "' + btoa(ReqXML.toString()) + '"}',
            ...
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2015","ServiceName" : "RTTIE039","ResponseXML" : "' + btoa(ResXML.toString()) + '"}',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (msg) {
    ...
    function DisplayNPTThankYou() {
        ...
        if (parent.$x.ispXmlGetFieldVal(parent.$g.xmlNPT_2015, 'NPT_INFO ADJUSTMENT_REF_NO', '', 0) != '') {
            $('#lblConfirmNo').text(parent.$x.ispXmlGetFieldVal(parent.$g.xmlNPT_2015, 'NPT_INFO ADJUSTMENT_REF_NO', '', ''));
            ...
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2015","ServiceName" : "RTTIE039"}',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (msg) {
    ...
    function PopulateNPTXML() {
		...
		parent.$x.ispXmlSetFieldVal(parent.$g.xmlNPT_2015, parent.$x.ispXmlGetFieldVal(parent.$g.xmlAccount, 'ENTITY_INFO ACCOUNT_ID', '', 0), 'NPT_INFO ACCOUNT_ID', '', 0); //Sudipta
    ...
    function CalcPage1Line2(rRate) {   //Line 1 times Resident Rate 
        ...
        //Line 1 times Resident Rate
        rRate = ".039102";
        var npt1 = parseCurrency($('#lblNPT1').text());

        if (npt1 <= 0) {
            $('#lblNPT2').text(ispFormatMoney(0, 0));
        } else {
            $('#lblNPT2').text(ispFormatMoney(npt1 * rRate, 0));
        } 	//if
    } //CalcPage1Line2
    ...
    function CalcPage1Line4(nrRate) {       ////Line 3 times Non-Resident Rate
        ...
        nrRate = ".034102";
        var npt3 = parseCurrency($('#lblNPT3').text());

        if (npt3 <= 0) {
            $('#lblNPT4').text(ispFormatMoney(0, 0));
        } else {
            $('#lblNPT4').text(ispFormatMoney(npt3 * nrRate, 0));
        } 	//if
    } //CalcPage1Line4
    function CalcPage1Line5() { //Line 2 Plus Line 4, Enter here and WrkC Line 1, WrkD Line 7 and WrkK Line 3
      
        //************************************************
        var npt2 = parseCurrency($('#lblNPT2').text());
        var npt4 = parseCurrency($('#lblNPT4').text());
        //
        $('#lblNPT5').text(ispFormatMoney(npt2 + npt4, 0));
    }
    ...
    function CalcPage1Line7() {   //New Line 7 times Base Non Resident Rate
       
        //************************************************************
        var nrRate = ".034828";
        var npt6 = parseCurrency($('#lblNPT6').text());

        if (npt6 <= 0) {
            $('#lblNPT7').text(ispFormatMoney(0, 0));
        } else {
            $('#lblNPT7').text(ispFormatMoney(npt6 * nrRate, 0));
        } 	//if
    }
    function CalcPage1Line9() {
        ...
        //***********************************************
        var nrRate = ".029828";
        var npt8 = parseCurrency($('#lblNPT8').text());
        if (npt8 <= 0) {
            $('#lblNPT9').text(ispFormatMoney(0, 0));
        } else {
            $('#lblNPT9').text(ispFormatMoney(npt8 * nrRate, 0));
        } 	//if
    }
    function CalcPage1Line10() {
      
        //******************************************************
        var npt7 = parseCurrency($('#lblNPT7').text());
        var npt9 = parseCurrency($('#lblNPT9').text());

        $('#lblNPT10').text(ispFormatMoney(npt7 + npt9, 0));
    }
    function CalcPage1Line11() { //Line 5 Plus Line 10, Enter here and WrkC Line 1, WrkD Line 7 and WrkK Line 3
        
        //************************************************************
        var npt5 = parseCurrency($('#lblNPT5').text());
        var npt10 = parseCurrency($('#lblNPT10').text());

        $('#lblNPT11').text(ispFormatMoney(npt5 + npt10, 0));
    ...
    //        $('#lblNPT15').text(ispFormatMoney((parseCurrency($('#lblNPT13').text()) + parseCurrency($('#lblNPT14').text())), 0));
    ...
    function CalcWrkALine4() { //Add Lines 1 thru 3 and enter on Page 1, Line 1
        ...
        wrkA1 = parseCurrency($('#txtNPT_WrkA_1').val());              
        wrkA2 = parseCurrency($('#txtNPT_WrkA_2').val());           
        wrkA3 = parseCurrency($('#txtNPT_WrkA_3').val());         

        $('#lblNPT_WrkA_4').text(ispFormatMoney(wrkA1 + wrkA2 + wrkA3, 0));
        //
        if ($('#txtNPT_WrkA_5').val() == 0) {
            $('#txtNPT_WrkA_5').val($('#lblNPT_WrkA_4').text());
        }		//if
    }

    function CalcWrkALine5_6() {
       
        wrkA5 = parseCurrency($('#txtNPT_WrkA_5').val());
        wrkA6 = parseCurrency($('#txtNPT_WrkA_6').val());
        if (parseCurrency($('#lblNPT_WrkA_4').text()) != 0) {
            if (wrkA5 + wrkA6 != parseCurrency($('#lblNPT_WrkA_4').text())) {
                ...
                $('#lblNPT1').text($('#txtNPT_WrkA_5').val());
                $('#lblNPT3').text($('#txtNPT_WrkA_6').val()); 
            }	//if
        }		//if
    }		//CalcWrkA5_6

    //*****End Page Worksheet A*****
    //******************************
    ...
    function CalcWrkBLine4() { //Add Lines 1 thru 3 and Enter on Page 4, Line 13
        ...
        wrkB1 = parseCurrency($('#txtNPT_WrkB_1').val());    
        wrkB2 = parseCurrency($('#txtNPT_WrkB_2').val()); 
        wrkB3 = parseCurrency($('#txtNPT_WrkB_3').val());     

        $('#lblNPT_WrkB_4').text(ispFormatMoney(wrkB1 + wrkB2 + wrkB3, 0));
    }

    function CalcWrkBLine6() { //Subtract Line 5 from Line 4 and enter on Page 1, Line 3
        
        //wrkB4 = parseCurrency($('#lblNPT_WrkB_4').text());  
        //wrkB5 = parseCurrency($('#lblNPT_WrkB_5').text());          

        //$('#lblNPT_WrkB_6').text(ispFormatMoney(wrkB4 - wrkB5, 0));
        //$('#txtNPT_WrkB_7').val($('#lblNPT_WrkB_6').text());
    ...
    function CalcWrkBLine7_8() {
        ...
        if (parseCurrency($('#lblNPT_WrkB_6').text()) != 0) {
            ...
            //else {
            //    $('#lblNPT6').text($('#txtNPT_WrkB_7').val());
            //    $('#lblNPT8').text($('#txtNPT_WrkB_8').val());  
            //} 	//if
        }		//if
    } 	//CalcWrkB7_8
    //*****End Worksheet B*****
    ...
    function CalcWrkDLine3() { //Divide Line 1 by 2 and enter the percentage here and on Page 1
        debugger;
        //******************************************************************
        var wrkD1 = parseCurrency($('#txtNPT_WrkD_1').val());
        var wrkD2 = parseCurrency($('#txtNPT_WrkD_2').val());
        if ($('#txtNPT_WrkD_1').val() != '' && $('#txtNPT_WrkD_2').val() != '') {
            $('#lblNPT_WrkD_3').text("0%");
            if (wrkD1 != 0 && wrkD2 == 0) {
                $('#AppError').text('Line 2 cannot be 0 if Line 1 is greater than 0 in Worksheet D');
                resolvedIframeheight();
                parent.ScrollTop(1);
            }
            else if (wrkD2 < wrkD1) {		//Not dividing by 0                
    ...
    function CalcWrkDLine5() { //Multiply the amount on Line 4 by the percentage on Line 3
        ...
        //**************************************************************************
        var wrkD3 = $('#lblNPT_WrkD_3').text().replace("%", "");
        var wrkD4 = parseCurrency($('#lblNPT_WrkD_4').text());
        $('#lblNPT_WrkD_5').text(ispFormatMoney((wrkD3 * wrkD4) / 100, 0));
    }
    ...
    function CalcWrkDLine6() { //Subtract Line 5 from Line 4
        ...
        //***************************************************************************
        var wrkD4 = parseCurrency($('#lblNPT_WrkD_4').text());
        var wrkD5 = parseCurrency($('#lblNPT_WrkD_5').text());
        $('#lblNPT_WrkD_6').text(ispFormatMoney(wrkD4 - wrkD5, 0));
    }

    function CalcWrkDLine8() { //Enter the lesser of Line 6 or Line 7 here and on Page 1, Line 12a :done
       
        //***********************************************************************
        var wrkD6 = parseCurrency($('#lblNPT_WrkD_6').text());
        var wrkD7 = parseCurrency($('#lblNPT_WrkD_7').text());
        ...
        //if (wrkD6 + wrkD7 != 0) {
        //    $('#lblNPT12a').text($('#lblNPT_WrkD_7').text());
        //}		//if
    }
    //*****End Worksheet D*****
    //********************************************
    ...
    function CalcWrkELine4() { //Total of Lines 1 through 3. Enter here and on Page 1, Line 12b
        ...
        //**********************************************************       
        var wrkE1 = parseCurrency($('#txtNPT_WrkE_1').val());  
        var wrkE2 = parseCurrency($('#txtNPT_WrkE_2').val());  
        var wrkE3 = parseCurrency($('#txtNPT_WrkE_3').val());
        if ($('#txtNPT_WrkE_1').val() != '' && $('#txtNPT_WrkE_2').val() != '' && $('#txtNPT_WrkE_3').val() != '') {
            $('#lblNPT_WrkE_4').text(ispFormatMoney(wrkE1 + wrkE2 + wrkE3, 0));
