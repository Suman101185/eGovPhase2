...
<script language="javascript" type="text/javascript">
    ...
    function DisplayNPTReturn() {
        ...
        if ((parent.GetNodeCount(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '24') == 1) && parent.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'PRK_ID', '24') != undefined && parent.GetInnerNodeText(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', 'PRK_ID', '24').slice(-4) > 2010) {
            ...
            if (parent.$x.ispXmlGetFieldVal(parent.$g.xmlBPTez, 'BPT_INFO VERSION', '', 0) == '') {
                $('#NPT_2014').css("display", "block");
                $('#NPT_Page1').css("display", "none");
                $('#NPT_BPT_msg').css("display", "block");
              
                alert('Your 2014 BIRT return must be filed prior to filing your 2014 NPT Return.' + '\n To fill out your 2014 BIRT form, Please select BIRT from the side menu.');
                return;
            }
        }
        if (parent.GetNodeCount(parent.$g.xmlAccount, 'TAX_ACCT ACCOUNT', '3') == 1) {
            ...
            parent.$x.ispXmlSetFieldVal(parent.$g.xmlNPT, '12/31/2014', 'NPT_INFO PERIOD', '', 0);



            var ReqXML = parent.$g.xmlNPT;
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2014","ServiceName" : "RTTIE039","RequestXML" : "' + btoa(ReqXML.toString()) + '"}',
            ...
            var ResXML = parent.$g.xmlNPT;
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2014","ServiceName" : "RTTIE039","ResponseXML" : "' + btoa(ResXML.toString()) + '"}',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (msg) {
            ...
            if (parent.$x.ispXmlGetFieldVal(parent.$g.xmlNPT, 'NPT_INFO CORP_PARTNER_IND', '', 0) != '') {
    ...
    function CheckNPTXML() {
        ...
        if (bPeriodExist == false) {
            ...
            parent.$x.ispXmlSetFieldVal(parent.$g.xmlNPT, 0, 'NPT_PG1 CORP_PARTNER_PERCENT', '', 0);
            //npt_wrka
    ...
    function DisplayNPTThankYou() {
        ...
        $('#NPT_Page4').css("display", "none");
        $('#NPT_2014').css("visibility", "hidden");
        $('#NPT_2014').css("display", "none");
        ...
        if (parent.$x.ispXmlGetFieldVal(parent.$g.xmlNPT, 'NPT_INFO ADJUSTMENT_REF_NO', '', 0) != '') {
            ...
            $.ajax({
                type: 'POST',
                url: '../../Returns/Log',
                data: '{ "OriginationFom" : "NPT-2014","ServiceName" : "RTTIE039"}',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (msg) {
        ...
        //resolved height of thanku page
    ...
    function PopulateNPTXML() {
        ...
        parent.$x.ispXmlSetFieldVal(parent.$g.xmlNPT, $('#lblPercentage').text().replace('%', ''), 'NPT_PG1 CORP_PARTNER_PERCENT', '', 0);
    ...
    function LoadError(formId) {        
        ...
        switch (formId) {
            ...
            case "WrkK":
                arrWrkKErr[i++] = [txtNPT_WrkK_1, 'ispValue($(\'#txtNPT_WrkK_1\').val()) == false', 'Value for question 1 must not be negative']

                break;
            case "WrkExtension":
                arrWrkExtErr[i++] = [txtNPT_WrkExtension_1, 'ispNegativeNumber($(\'#txtNPT_WrkExtension_1\').val()) == false', 'Value for question 1 must not be negative']
                arrWrkExtErr[i++] = [txtNPT_WrkExtension_2, 'ispNegativeNumber($(\'#txtNPT_WrkExtension_2\').val()) == false', 'Value for question 2 must not be negative']
    ...
    function ValidateWorksheet(formId) {
        ...
        switch (formId) {
            ...
            case "WrkD":
                ...
                if (PrepForMath($('#txtNPT_WrkD_8').val()) > PrepForMath($("#lblNPT_WrkD_6").text())) {
                    if (PrepForMath($('#txtNPT_WrkD_8').val()) > PrepForMath($("#lblNPT_WrkD_7").text())) {
                        $("#AppError").text('Line 8 value must be lower of of Line 6 or Line 7');
                    }
            ...
            case "WrkK":
                ...
                if (PrepForMath($('#txtNPT_WrkK_4').val()) > PrepForMath($("#lblNPT_WrkK_2").text())) {
                    if (PrepForMath($('#txtNPT_WrkK_4').val()) > PrepForMath($("#lblNPT_WrkK_3").text())) {
                        $("#AppError").text('Line 4 value must be lower of of Line 2 or Line 3');
                    }
    ...
    function chkCorpMembers_onclick() { //Determine if questions 3 & 4 of Worksheet K are needed or if Worksheet D will be used instead
        ...
        //added 12.30.16
        if (chkCorpMembers.checked == true) {
    ...
    function CalcPage1Line6a() {
        if ($('#txtNPT_WrkD_8').val() != '') {
            $('#lblNPT6a').text($('#txtNPT_WrkD_8').val());
        }
        if ($('#lblNPT6a').text() != '') {
    ...
    function CalcPage1Line7() {
        ...
        if ($('#lblNPT5').text() != '' && $('#lblNPT6c').text() != '') {
            ...
            if ((parseCurrency($('#lblNPT5').text()) - parseCurrency($('#lblNPT6c').text())) > 0) { //Net Tax Due is Positive
    ...
    function CalcPage1Line12() {
        if (parseCurrency($('#lblNPT10').text()) > parseCurrency($('#lblNPT11').text())) {
            if ($('#txtNPT12a').val() == '') {
    ...
    function CalcWrkBLine6() { //Subtract Line 5 from Line 4 and enter on Page 1, Line 3
        if ($('#lblNPT_WrkB_4').text() != '' && $('#lblNPT_WrkB_5').text() != '') {
            ...
            $('#lblNPT3').text($('#lblNPT_WrkB_6').text());
        }
    }
    //*****End Worksheet B*****
    //*****Worksheet C*****
